...
| `.claude/skills/<name>` | Symlink → `../../.agents/skills/<name>` (lido por Claude Code) |
...
Claude Code não lê `.agents/skills/` direto (issue #22902) — por isso os symlinks em `.claude/skills/` existem. Git versiona symlinks (modo 120000), então tudo viaja junto no clone.

**Adicionar/atualizar/remover skill via upstream:**

```bash
npx skills add <source> --agent claude-code            # baixa do GitHub para .agents/skills/ + linka .claude/skills/
...
git add .agents/skills/ .claude/skills/ .codex/skills/ skills-lock.json
```

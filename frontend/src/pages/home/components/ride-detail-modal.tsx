...
import type { Ride, Similarity } from "../types";
...
export function RideDetailModal({
  ...
  if (!ride) return null;
  const sim = similarityBanner[ride.similarity];
      ...
      >
        ...
        <div className="flex flex-col gap-3 px-4 pb-4">
          ...
          <div className="grid grid-cols-3 gap-2 rounded-2xl border border-border bg-card p-3">
            <InfoCell icon={Clock01Icon} label="Horário" value={ride.time} />
            <InfoCell
              icon={UserMultiple02Icon}
              label="Vagas"
              value={`${ride.seatsTaken}/${ride.seatsTotal}`}
            />
            <InfoCell
              label="R$"
              value={formatPrice(ride.priceBRL)}
          ...
          >
            ...
            <div className="flex flex-col gap-0.5 text-[12px]">
              <p className="font-bold">
                {sim.label} · {ride.similarityMatchPct}% do trajeto
              </p>
